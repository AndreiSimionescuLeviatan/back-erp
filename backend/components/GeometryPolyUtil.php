...
class GeometryPolyUtil
{
    ...
    private static function isLocationOnEdgeOrPath($point, $poly, $closed, $geodesic, $toleranceEarth)
    {
        ...
        $tolerance = $toleranceEarth / GeometryMathUtil::EARTH_RADIUS;
        $havTolerance = GeometryMathUtil::hav($tolerance);
        $lat3 = deg2rad($point['lat']);
        $lng3 = deg2rad($point['lng']);
        ...
        if ($geodesic) {
            ...
            $y1 = GeometryMathUtil::mercator($lat1);
            $y3 = GeometryMathUtil::mercator($lat3);
            $xTry = [];
            foreach ($poly as $val) {
                $lat2 = deg2rad($val['lat']);
                $y2 = GeometryMathUtil::mercator($lat2);
                $lng2 = deg2rad($val['lng']);
                if (max($lat1, $lat2) >= $minAcceptable && min($lat1, $lat2) <= $maxAcceptable) {
                    // We offset longitudes by -lng1; the implicit x1 is 0.
                    $x2 = GeometryMathUtil::wrap($lng2 - $lng1, -M_PI, M_PI);
                    $x3Base = GeometryMathUtil::wrap($lng3 - $lng1, -M_PI, M_PI);
                    $xTry[0] = $x3Base;
                    ...
                    foreach ($xTry as $x3) {
                        $dy = $y2 - $y1;
                        $len2 = $x2 * $x2 + $dy * $dy;
                        $t = $len2 <= 0 ? 0 : GeometryMathUtil::clamp(($x3 * $x2 + ($y3 - $y1) * $dy) / $len2, 0, 1);
                        $xClosest = $t * $x2;
                        $yClosest = $y1 + $t * $dy;
                        $latClosest = GeometryMathUtil::inverseMercator($yClosest);
                        $havDist = GeometryMathUtil::havDistance($lat3, $latClosest, $x3 - $xClosest);
    ...
    private static function sinDeltaBearing($lat1, $lng1, $lat2, $lng2, $lat3, $lng3)
    {
        ...
        $a = sin($lng31) * $cosLat3;
        $c = sin($lng21) * $cosLat2;
        $b = sin($lat31) + 2 * $sinLat1 * $cosLat3 * GeometryMathUtil::hav($lng31);
        $d = sin($lat21) + 2 * $sinLat1 * $cosLat2 * GeometryMathUtil::hav($lng21);
    ...
    private static function isOnSegmentGC($lat1, $lng1, $lat2, $lng2, $lat3, $lng3, $havTolerance)
    {
        ...
        $havDist13 = GeometryMathUtil::havDistance($lat1, $lat3, $lng1 - $lng3);
        if ($havDist13 <= $havTolerance) {
            return true;
        }
        $havDist23 = GeometryMathUtil::havDistance($lat2, $lat3, $lng2 - $lng3);
        if ($havDist23 <= $havTolerance) {
            return true;
        }
        $sinBearing = self::sinDeltaBearing($lat1, $lng1, $lat2, $lng2, $lat3, $lng3);
        $sinDist13 = GeometryMathUtil::sinFromHav($havDist13);
        $havCrossTrack = GeometryMathUtil::havFromSin($sinDist13 * $sinBearing);
        if ($havCrossTrack > $havTolerance) {
            return false;
        }
        $havDist12 = GeometryMathUtil::havDistance($lat1, $lat2, $lng1 - $lng2);
        ...
        $sinSumAlongTrack = GeometryMathUtil::sinSumFromHav($havAlongTrack13, $havAlongTrack23);
    ...
    {
        if ($start == $end) {
            return GeometrySphericalUtil::computeDistanceBetween($end, $p);
        }
        $s0lat = deg2rad($p['lat']);
        $s0lng = deg2rad($p['lng']);
        ...
        if ($u <= 0) {
            return GeometrySphericalUtil::computeDistanceBetween($p, $start);
        }
        if ($u >= 1) {
            return GeometrySphericalUtil::computeDistanceBetween($p, $end);
        }
        $sa = array('lat' => $p['lat'] - $start['lat'], 'lng' => $p['lng'] - $start['lng']);
        $sb = array('lat' => ($u * ($end['lat'] - $start['lat'])), 'lng' => ($u * ($end['lng'] - $start['lng'])));
        return GeometrySphericalUtil::computeDistanceBetween($sa, $sb);
    }
