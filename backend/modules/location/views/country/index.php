...
$this->title = Yii::t('location', 'Countries');
$this->params['breadcrumbs'][] = $this->title;

$hasPermission = AppHelper::checkPermissionViewDeletedEntities($_GET['CountrySearch']['deleted'] ?? '', 'activateCountry');
$toggleStatus = !$hasPermission ? 'checked' : '';
$activeDeletedEntities = isset($_GET['CountrySearch']['deleted']) && $_GET['CountrySearch']['deleted'] == 1 ? 1 : 0;
$viewEntitiesToggle = '<label class="personalized-toggle mb-0 mt-1">' .
    ...
    '</label>' .
    '<input type="hidden" id="toggle-status" name="CountrySearch[deleted]" value="' . $activeDeletedEntities . '">';
?>
    ...
    <?php echo GridView::widget([
        ...
        'columns' => [
            ...
            [
                ...
                'filter' => CountrySearch::$userAdded,
...
$this->registerJs(
    "viewToggleChange('CountrySearch[deleted]');",
    View::POS_READY,
    'view-toggle-change-handler'
);

$this->registerJs(
    "setPageSize('CountrySearch[pageSize]');",
    View::POS_READY,
    'grid-view-page-size'
);
?>
